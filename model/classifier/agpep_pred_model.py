...
class AgpepPredModel(pl.LightningModule):
    def __init__(self, input_dim=320, hidden_dim=256, num_classes=2,
                 learning_rate=1e-4):
        super().__init__()
    ...
    def forward(self, batch):
        batch_size = len(batch['pep'])
        pep_seq_data = [(x, seq) for x, seq in zip(range(batch_size), batch['pep'])]
        ...
        for i in range(batch_size):
            ...
            sequence_representations.append(pep_repre)
    ...
    @torch.no_grad()
    def inference_batch(self, pep_seq_list):
    ...
    def inference_from_onehot(self, padding_mask, pep_token_onehot):

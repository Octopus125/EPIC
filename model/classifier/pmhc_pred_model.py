# p-MHC presentation prediction model
import torch
...
class PMHCPredModel(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        mhc_seq_data = [(x, seq) for x, seq in zip(range(batch_size), batch['mhc_seq'])]
        pep_seq_data = [(x, seq) for x, seq in zip(range(batch_size), batch['pep_seq'])]

        _, _, mhc_batch_tokens = self.batch_converter(mhc_seq_data)
        mhc_batch_tokens = mhc_batch_tokens.to(self.device)
        mhc_batch_lens = (mhc_batch_tokens != self.alphabet.padding_idx).sum(1)

        results = self.esm_model(mhc_batch_tokens, repr_layers=[6], return_contacts=True)
        mhc_token_representations = results["representations"][6]
        ...
        for i in range(batch_size):
            mhc_token_len = mhc_batch_lens[i]
            pep_token_len = pep_batch_lens[i]
            mhc_repre = mhc_token_representations[i, 1 : mhc_token_len - 1].mean(0)
            pep_repre = pep_token_representations[i, 1 : pep_token_len - 1].mean(0)
            sequence_representations.append(torch.cat((mhc_repre, pep_repre)))
    ...
    @torch.no_grad()
    def inference_batch(self, mhc_seq, pep_seq_list):
        batch_size = len(pep_seq_list)
        mhc_seq_data = [('mhc', mhc_seq)]
        ...
        mhc_batch_lens = (mhc_batch_tokens != self.alphabet.padding_idx).sum(1)
        results = self.esm_model(mhc_batch_tokens, repr_layers=[6], return_contacts=True)
        mhc_token_representations = results["representations"][6] # [1, mhc_len, 320]
        ...
        sequence_representations = []
        mhc_token_len = mhc_batch_lens[0]
        mhc_repre = mhc_token_representations[0, 1 : mhc_token_len - 1].mean(0) # [320]
    ...
    def inference_from_onehot(self, mhc_seq, padding_mask, pep_token_onehot):
